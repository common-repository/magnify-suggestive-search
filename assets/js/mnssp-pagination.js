jQuery(document).ready(function($) {
    ...
    $(window).scroll(function () {
        if ($(window).scrollTop() + $(window).height() >= $(document).height() - 200 && !isLoading) {
            ...
            jQuery('.mnssp-header-border-bottom').show();
    ...
    function productsAjax( endCursor, templateSearch, collection, actionValue ) {
        ...
        var progressInterval = setInterval(function() {
            ...
            if (progress >= 100) {
                ...
                $('.mnssp-header-border-bottom').css('width', '100%');
            } else {
                $('.mnssp-header-border-bottom').css('width', progress + '%');
            }
        }, 300);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                jQuery('.mnssp-header-border-bottom').hide();
